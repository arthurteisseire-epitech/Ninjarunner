...
var ninja = {
    ...
    update: function () {
        ...
        if (mort == false) {
            ...
            if (iCanLaunchShuriken() && shuriken.x < -32)
            {
            ...
            if (iCanLaunchPique() && shuriken.x < -32)
            {
                pique = game.add.sprite(1300, 350,'pique');
                game.physics.arcade.enable(pique, Phaser.Physics.ARCADE);
                pique.body.bounce.set(1);
                pique.body.allowGravity = false;
                pique.body.velocity.x = objectSpeed;
            ...
            if (playerEstAuSol || this.cursors.up.isDown) {
...
function iCanLaunchShuriken ()
{
    ...
    if (interframeShuriken > limitInterframeShuriken){
...
{
    interframePique +=1;
    if (interframePique > limitInterframePique){
        interframePique = 0;
